...
def train_model(config, epochs, batch_size=64, sentence_max_len=64, lr=1e-4):
    ...
    train_x_vgenome = pkl.load(open("/home/user/palme/LabLanguageVision/csi_code/data_x_vgenome.pkl", "rb"))
    train_x_vgenome_ranges = pkl.load(open("/home/user/palme/LabLanguageVision/csi_code/data_x_vgenome_ranges.pkl", "rb"))
        ...
        for i in tqdm.tqdm(range(0, len(train_x), batch_size)):
            ...
            if epoch >= num_warmup_epochs:
                ...
                if k + batch_size > len(train_x_vgenome):
                    inputs = train_x_vgenome[k:]
                    ranges = [train_x_vgenome_ranges[t] for t in range(k, len(train_x_vgenome))]
                    k = 0
                else:
                    inputs = train_x_vgenome[k:k + batch_size]
                    ranges = [train_x_vgenome_ranges[t] for t in range(k, k + batch_size)]
                    k += 1
                if any(len(sublist) > sentence_max_len for sublist in inputs):
                    inputs = [x[:sentence_max_len] for x in inputs]

                # used for zeroing the labels tensor later on
                lengths = [len(t) for t in inputs]
                max_len = max(lengths)

                # find related region descriptions for every input
                matching_indices1 = [rnd.randint(lo, hi) for lo, hi in ranges]
                matching_indices2 = [rnd.randint(lo, hi) for lo, hi in ranges]
                inputs_rnd1 = [train_x_vgenome[t] for t in matching_indices1]
                inputs_rnd2 = [train_x_vgenome[t] for t in matching_indices2]

                # append related region descr. to the input region descr.
                inputs_appended1 = [inputs[t] + ['.'] + inputs_rnd1[t] for t in range(len(inputs))]
                inputs_appended2 = [inputs[t] + ['.'] + inputs_rnd2[t] for t in range(len(inputs))]

                # apply model to appended sentences
                #outputs1 = torch.exp(model.eval()(inputs_appended1))[:, :max_len, :]
                #outputs2 = torch.exp(model.eval()(inputs_appended2))[:, :max_len, :]
                outputs1 = model.eval()(inputs_appended1)[:, :max_len, :]
                outputs2 = model.eval()(inputs_appended2)[:, :max_len, :]
                labels = (outputs1 + outputs2) / 2.

                # sharpening
                T = 0.5
                labels = torch.pow(labels, 1./T)
                std = torch.sum(labels, dim=2)
                labels.div_(std[:, :, None]) # None is numpy-indexing that does "np.newaxis"


                optimizer.zero_grad()
                #outputs = torch.exp(model.train()(inputs))
                outputs = model.train()(inputs)

                # make outputs and labels identical on outputs that don't
                # matter, so that the loss is 0 (not sure if this is good...)
                for i, length in enumerate(lengths):
                    labels[i, length:max_len, :] = outputs[i, length:max_len, :]
